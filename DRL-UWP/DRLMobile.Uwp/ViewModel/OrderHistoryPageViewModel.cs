...
namespace DRLMobile.Uwp.ViewModel
{
    public class OrderHistoryPageViewModel : BaseModel
    {
        ...
        private bool IsOrderHistoryDetailPageVisit { get; set; }
        ...
        public ICommand DeleteOrderHistoryCommand { get; private set; }
        #endregion
        ...
        public OrderHistoryPageViewModel()
        {
            ...
            DeleteOrderHistoryCommand = new AsyncRelayCommand<OrderHistoryUIModel>(DeleteOrderHistoryCommandHandler);
            IsFirstLoad = true;
        ...
        private async Task DeleteOrderHistoryCommandHandler(OrderHistoryUIModel arg)
        {
            try
            {

                var result = await AlertHelper.Instance.ShowConfirmationAlert("Alert", "Are you sure that you want to delete this Order?", "Yes", "No");
                ...
                {
                    // delete activity
                    await AppReference.QueryService.DeleteActivityFromDeviceOrderId(arg?.DeviceOrderID);

                    var isDetailsDeleted = await AppReference.QueryService.DeleteOrderDetailsFromDeviceOrderId(arg?.DeviceOrderID);

                    if (isDetailsDeleted)
                    {
                        // delete order details
                        var isOrderDeleted = await AppReference.QueryService.DeleteOrderMasterDataFromOrderIdAndDeviceOrderId(arg.OrderId, arg?.DeviceOrderID);

                        if (isOrderDeleted)
                        {
                            // delete order master
                            await AlertHelper.Instance.ShowConfirmationAlert("Success", "Order Deleted Successfully", "OK");
                            OrderHistoryItemSource.Remove(arg);

                            // remove signature file
                            await RemoveSignatureFromStorage(arg.SignaturePath);
                        }
                        else
                        {
                            await AlertHelper.Instance.ShowConfirmationAlert("Failure", "Error while deleting Order", "OK");
                        }
                    }
                    else
                    {
                        await AlertHelper.Instance.ShowConfirmationAlert("Error", "Error while deleting order details", "OK");
                    }
                }
            }
            catch (Exception ex)
            {
                ErrorLogger.WriteToErrorLog(nameof(OrderHistoryPageViewModel), nameof(DeleteOrderHistoryCommandHandler), ex.StackTrace + " - " + ex.Message);
            }
            finally
            {
                IsLoading = false;
        ...
        private async Task RemoveSignatureFromStorage(string signaturePath)
        {
            var isExist = File.Exists(signaturePath);
            if (isExist)
            {
                var file = await StorageFile.GetFileFromPathAsync(signaturePath);
                if (file != null)
                {
        ...
        private async Task OnNavigatedToCommandHandler(object parameter)
        {
            try
            {
                ...
                else if (parameter is OrderDetailUpdatedModel)
                {
                    ...
                    {
                        OrderHistoryItemSource.FirstOrDefault(x => x.OrderId == order.OrderId).TotalAmount = "$" + order.Price;
            ...
            {
                var errDesc = ex.InnerException.Message + ", " + ex.InnerException.StackTrace + ", " + ex.StackTrace;

                ErrorLogger.WriteToErrorLog(nameof(OrderHistoryPageViewModel), nameof(OnNavigatedToCommandHandler), errDesc);
                IsLoading = false;
                IsFirstLoad = false;
            }
        }
        ...
        private async Task ItemClickCommandHandler(OrderHistoryUIModel selectedObject)
        {
            if (!IsReorderTapped)
            {
                ...
                IsOrderHistoryDetailPageVisit = true;
                NavigationService.NavigateShellFrame(typeof(OrderHistoryDetailsPage), navObj);
            }
            else
            {
                IsLoading = true;
        ...
        private async Task ReorderClicked(OrderHistoryUIModel selectedObject)
        {
            ...
            if (result)
            {
                try
                {
                    ...
                    else if ("5".Equals(selectedObject.SalesType))
                    {
                        AppReference.CartDataFromScreen = 1;
                    }
                    else if ("10".Equals(selectedObject.SalesType))
                    {
                        AppReference.CartDataFromScreen = 2;
                    }
                    else
                    {
                    ...
                    var details = await AppReference.QueryService.GetOrderDetailsFromOrderId(selectedObject.DeviceOrderID);
                    if (AppReference.CartDataFromScreen == 1) //Rack Order Sales
                    {
                        details = details.OrderBy(x => x.OrderDetailId);
                    }
                    await AppReference.QueryService.DeleteCartDetailOnPlaceOrder(AppReference.CurrentDeviceOrderId);

                    // Before Sync
                    if (selectedObject.IsExported == 0)
                    {
                        details = details.OrderBy(x => DateTimeHelper.ConvertToDBDateTime(x.CreatedDate)).ToList();
                    }
                    //After Sync
                    else
                    {
                        details = details.OrderBy(x => x.OrderDetailId).ToList();
                    }

                    foreach (var item in details)
                    {
                        item.OrderId = AppReference.CurrentOrderId;
                        item.DeviceOrderID = AppReference.CurrentDeviceOrderId;
                        item.OrderDetailId = null;
                        item.CreatedDate = DateTimeHelper.ConvertToDbInsertDateTimeMilliSecondFormat(DateTime.Now);
                    ...
                    if (!"5".Equals(selectedObject.SalesType) && !"10".Equals(selectedObject.SalesType))
                    {
        ...
        {
            IsLoading = true;
            if (IsOrderHistoryDetailPageVisit == true)
            {
                DBItemHistorySource = await AppReference.QueryService.GetItemHistoryDataAsync(SelectedCustomer.DeviceCustomerID, SelectedCustomer.AccountType.ToString());
                ItemHistoryItemSource?.Clear();
            ...
            else
            {
                if (DBItemHistorySource != null && DBItemHistorySource.Any())
                {
                    if (ItemHistoryItemSource.Count == 0)
                    {
                        //ItemHistoryItemSource?.Clear();
                        foreach (var item in DBItemHistorySource)
                        {
            ...
            IsOrderHistoryDetailPageVisit = false;
            IsLoading = false;
        }
        ...
        private async Task GetTheOrderHistoryData()
        {
            try
            {
                ...
                {
                    custId = SelectedCustomer.DeviceCustomerID;
                    custType = SelectedCustomer.AccountType.ToString();

                }
                if (IsOrderHistoryDetailPageVisit == true)
                {
                    DBOrderHistorySource = await AppReference.QueryService.GetOrderHistoryDataAsync(custId, custType);

                    DBOrderHistorySource?.ToList().ForEach(x => OrderHistoryItemSource.Add(x));

                    OrderHistoryVisibility();

                    DBItemHistorySource = await AppReference.QueryService.GetItemHistoryDataAsync(custId, custType);

                    IsOrderHistoryDetailPageVisit = false;
                }
                else
                {
                    if (DBOrderHistorySource != null && DBOrderHistorySource.Any())
                    {
                        DBOrderHistorySource.ToList().ForEach(x => OrderHistoryItemSource.Add(x));
                        OrderHistoryVisibility();
                        DBItemHistorySource = await AppReference.QueryService.GetItemHistoryDataAsync(custId, custType);
            ...
            catch (Exception ex)
            {
                ...
                ErrorLogger.WriteToErrorLog(nameof(OrderHistoryPageViewModel), nameof(GetTheOrderHistoryData), errDesc);
        ...
        private void HandleHeaderTextChangeForLeftSegment(string text)
        {
            try
            {
                ...
                {
                    List<OrderHistoryUIModel> finalList = new List<OrderHistoryUIModel>();

                    var tempList = DBItemHistorySource.Where(x => x.SearchDisplayPathForOrderScreen.ToLower().Contains(text.ToLower())).ToList();

                    if (tempList == null || tempList.Count == 0)
                    {
                        finalList = DBOrderHistorySource.Where(x => x.SearchDisplayPath.ToLower().Contains(text.ToLower())).ToList();
                    }
                    else
                    {
                        finalList = DBOrderHistorySource.Where(x => tempList.Any(y => y.Invoice.Equals(x.Invoice))).ToList();
                    }
                    ...
                    {
                        finalList.ForEach(x => HeaderSearchItemSource.Add(x));
                    }
                }
            }
            catch (Exception ex)
