...
namespace DRLMobile.Uwp.ViewModel
{
    public class ViewGoogleRoutePageViewModel : BaseModel
    {
        ...
        #region Object/Command Initialization
        ...
        private void RegisterCommand()
        {
            ...
            SelectAllCommand = new AsyncRelayCommand(SelectAllCommandHandler);
        }
        ...
        private async void OnCheckBoxClickedHandler(ViewRouteDetailsUIModel customer)
        {
            if (customer != null)
            {
                ...
                {
                    var boolcheck = !selectedCustomer.IsChecked;
                    ...
                    selectedCustomer.IsChecked = !selectedCustomer.IsChecked;
        ...
        private async Task SelectAllCommandHandler()
        {
        ...
        private async Task OnNavigatedToCommandHandler(RouteListUIModel selectedRoute)
        {
            ...
            if (RouteDetailsDBSource != null && RouteDetailsDBSource.Count > 0)
            {
                ...
                foreach (var item in RouteDetailsDBSource)
                {
                    ...
                    var newItem = CloneRouteDetails(item);
                    newItem.ListIndex = indexItem; newItem.IsChecked = false;
                    RouteDetailsItemSource.Add(newItem);
                    indexItem++;
                }
            }
        ...
        private async Task HeaderSearchTextChangeCommandHandler(string SearchText)
        {
            ...
            else
            {
                ...
                {
                    // FIXED: Create new instances to avoid reference sharing
                    foreach (var item in tempList)
                    {
                        HeaderSearchItemSource.Add(CloneRouteDetails(item));
                    }
                }
            }
        }
        ...
        private void SuggestionChoosen(ViewRouteDetailsUIModel selectedItem)
        {
            ...
            // FIXED: Create a new instance instead of adding the reference directly
            if (_filterItem != null)
            {
                RouteDetailsItemSource.Add(CloneRouteDetails(_filterItem));
            }
            UpdateSelectAllCheckboxState();
        }
        ...
        private async Task FindDistantWaypointsAsync(List<EnhancedWaypoint> waypoints, double maxDistanceKm = 300.0)
        {
        ...
        private async Task NavigationButtonCommandHandler()
        {
            ...
            try
            {
                ...
                // === Step 2: Generate Google URLs (using addresses if available, split if needed) ===
                List<string> googleMapsUrls = BuildGoogleMapsUrlsWithAddresses(startInfo, waypointInfos, endInfo);

                // === Step 3: Get user info & send email ===
        ...
        /// <summary>
        /// Creates a shallow copy of a ViewRouteDetailsUIModel to prevent shared reference state across collections.
        /// </summary>
        private ViewRouteDetailsUIModel CloneRouteDetails(ViewRouteDetailsUIModel source)
        {
            ...
            return new ViewRouteDetailsUIModel
            {
                ...
                PhysicalAddressStateID = source.PhysicalAddressStateID,
                PhysicalAddressZipCode = source.PhysicalAddressZipCode,
