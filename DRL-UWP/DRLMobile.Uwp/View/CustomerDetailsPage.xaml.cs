...
namespace DRLMobile.Uwp.View
{
    ...
    {
        public CustomerDetailsPageViewModel ViewModel = new CustomerDetailsPageViewModel();
        private object NavigationArgument = null;
        ...
        private async void CustomerDetailsPage_Loaded(object sender, RoutedEventArgs e)
        {
            await FocusManager.TryFocusAsync(CustomerNameTextBox, FocusState.Programmatic);
            if (ViewModel != null)
            {
                //ViewModel.IsLoading = true;
                await ViewModel.OnNavigatedToCommandAsync.ExecuteAsync(NavigationArgument);

                //ViewModel.IsLoading = false;
                ViewModel.FlyoutEvent += FlyoutOpen;

                // Update positioning of ChainPlaybook button based on visibility of other buttons
                if (ViewModel.UiModel.IsChainPlaybookAvailable)
                {                    
                    UpdateChainPlaybookPosition();
                }
            }
        }

        private void UpdateChainPlaybookPosition()
        {
            // Always position below RackOrderButton
            RelativePanel.SetBelow(BtnChainPlaybook, RackOrderButton);

            if (ViewModel.UiModel.IsDirectCustomer)
            {
                // Position right of CreditRequest if visible
                RelativePanel.SetRightOf(BtnChainPlaybook, CreditRequest);
            }
            else
            {
                // If neither is visible
                RelativePanel.SetRightOf(BtnChainPlaybook, PopOrderButton);
            }
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            NavigationArgument = e.Parameter;
        }
        ...
        private void FlyoutOpen(object sender, Core.Enums.CustomerDetailFlyoutType e)
        {
            switch (e)
            {
                ...
                case Core.Enums.CustomerDetailFlyoutType.ContactEmail:
                    AlertFlyoutTextBlock.Text = "Email id is mandatory. If Customer doesn’t provide one, please enter noemail@republicbrands.com";
                    AlertFlyout.ShowAt(HeaderControl);
                    break;
        ...
        private void DistributorDeletedIconTapped(object sender, TappedRoutedEventArgs e) => ViewModel.UiModel.DeleteDistributorButtonCommand.Execute((sender as Grid).DataContext as DistributorMaster);
